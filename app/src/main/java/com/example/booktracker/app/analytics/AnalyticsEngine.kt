...
fun QualitativeRatingSliders() {
    var pacing by remember { mutableStateOf(2.5f) }
    var focus by remember { mutableStateOf(2.5f) }
    var vibe by remember { mutableStateOf(2.5f) }
...
class AnalyticsEngine {
    fun calculateReadingVelocity(session: Session): Float {
        if (session.startTime == null || session.endTime == null) return 0f
        val durationMinutes = (session.endTime!!.seconds - session.startTime!!.seconds) / 60f
